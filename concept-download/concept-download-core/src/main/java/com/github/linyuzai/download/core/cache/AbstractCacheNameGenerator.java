...
public abstract class AbstractCacheNameGenerator implements CacheNameGenerator {
     ...
     */
    @Override
    public String generate(DownloadableResource downloadableResource, DownloadContext context) {
        String name = downloadableResource.getName();
        if (StringUtils.hasText(name)) {
            return name;
        } else {
            return doGenerate(downloadableResource, context);
        }
    }
     ...
     * @return 自定义生成的名称
     */
    public abstract String doGenerate(DownloadableResource downloadableResource, DownloadContext context);
}
