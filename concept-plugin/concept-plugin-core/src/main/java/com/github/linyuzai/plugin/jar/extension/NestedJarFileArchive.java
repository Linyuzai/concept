...
/**
 * {@link Archive} implementation backed by a {@link NestedJarFile}.
 *
 * @author Phillip Webb
 * @author Andy Wilkinson
 * @since 1.0.0
 */
public class NestedJarFileArchive implements Archive {
	...
	public NestedJarFileArchive(File file) throws IOException {
		this(file, file.toURI().toURL());
	}

	public NestedJarFileArchive(File file, URL url) throws IOException {
		this(new NestedJarFile(file));
		this.url = url;
	}

	public NestedJarFileArchive(NestedJarFile jarFile) {
	...
	protected Archive getNestedArchive(Entry entry) throws IOException {
		...
		try {
			...
			return new NestedJarFileArchive(jarFile);
		}
		catch (Exception ex) {
	...
	private Archive getUnpackedNestedArchive(JarEntry jarEntry) throws IOException {
		...
		return new NestedJarFileArchive(path.toFile(), path.toUri().toURL());
	}
