...
public class BlockingChainFactory<T, CTX extends Context, CH extends Chain<T, CTX>> implements ChainFactory<T, CTX, CH> {

    @SuppressWarnings("unchecked")
    @Override
    public CH create(int index, List<? extends Invoker<T, CTX, CH>> invokers, Function<CTX, Return<T>> function) {
        return (CH) new BlockingChain<>(index, invokers, function);
    }
}
